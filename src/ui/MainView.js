// src/ui/MainView.js - Full UI Restore with Refresh & Radar (Full-Body)

import React, { useRef, useEffect } from 'react';
import { StatusBar, StyleSheet, Text, View, TouchableOpacity, ScrollView, Platform, UIManager, Animated, ActivityIndicator, Alert } from 'react-native';
...
export default function MainView() {
  ...
  const { settings, macroData, finData, assets, dialogs, fontsLoaded, isLoading } = state;
  ...
  const confirmRefresh = () => {
    Alert.alert(
      "Analyse erneuern",
      "Möchtest du den Cache wirklich löschen und eine neue KI-Analyse starten?",
      [
        { text: "Abbrechen", style: "cancel" },
        { text: "Ja, erneuern", onPress: actions.handleForceRefresh, style: "destructive" }
      ]
    );
  };
  ...
  const dynamicStyles = StyleSheet.create({
    ...
    refreshBtn: { padding: 4, borderRadius: 6, borderWidth: 1, borderColor: '#ffbb33', marginLeft: 4, opacity: isLoading ? 0.5 : 1 },
        ...
        <SafeAreaView style={dynamicStyles.container}>
          ...
          <View style={dynamicStyles.toolbar}>
            <TouchableOpacity style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }} onPress={() => actions.toggleDialog('macro', true)}>
              ...
              <Text style={{ color: currentTheme.colors.text, fontWeight: '500' }}>{isLoading ? 'Analysiere...' : 'Market'}</Text>
            </TouchableOpacity>
            
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
              <TouchableOpacity onPress={() => actions.toggleDialog('radar', true)} style={{ padding: 8 }}>
                {fontsLoaded && <Ionicons name="scan-outline" size={24} color={currentTheme.colors.text} />}
              </TouchableOpacity>

              <TouchableOpacity onPress={confirmRefresh} style={dynamicStyles.refreshBtn} disabled={isLoading}>
                {fontsLoaded && <Ionicons name="refresh-outline" size={18} color="#ffbb33" />}
              </TouchableOpacity>
          ...
          <ScrollView contentContainerStyle={{ padding: 16 }}>
            {macroData?.error && (
              <View style={dynamicStyles.errorBox}>
                <Text style={{ color: currentTheme.colors.error, fontWeight: 'bold', fontSize: 11 }}>STATUS-MELDUNG</Text>
                <Text style={{ color: currentTheme.colors.text, fontSize: 12 }}>{macroData.error}</Text>
              </View>
            )}
            ...
            <View style={{ marginBottom: 16, flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
               <Text style={{ color: currentTheme.colors.text, fontWeight: 'bold', fontSize: 18 }}>Portfolio</Text>
               <TouchableOpacity onPress={() => actions.toggleDialog('addAsset', true)}>
                  <Ionicons name="add-circle" size={32} color={currentTheme.colors.primary} />
               </TouchableOpacity>
            </View>

            {assets.length === 0 ? (
              <Text style={{ color: currentTheme.colors.textSubtle, textAlign: 'center', marginTop: 40 }}>Portfolio leer.</Text>
            ) : (
              assets.map(asset => (
                <StockItem 
                  key={asset.ticker} 
          ...
          {/* Dialogs */}
          <StockRadarDialog visible={dialogs.radar} onClose={() => actions.toggleDialog('radar', false)} />
