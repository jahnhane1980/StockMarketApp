...
export class SettingsRepository {
  static async getSettings() {
    ...
    const currentKey = STORAGE_KEYS.settings(); // NEU: Zentrale Key-Registry
  ...
  static async saveSettings(settings) {
    ...
    const storage = StorageServiceFactory.getPersistentService();
    const currentKey = STORAGE_KEYS.settings(); // NEU
    
    try {
      const currentSettings = await this.getSettings();
      const newSettings = { ...currentSettings, ...settings };
      await storage.setItem(currentKey, JSON.stringify(newSettings));
